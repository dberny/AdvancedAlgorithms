...
def tsp(adjList, start):
    ...
    while len(cities) > 0:
        ...
        # print(currentCity.rank)
        tour.append(currentCity.rank)

        for neighbor in currentCity.mstN:
...
class Map:
    ...
    def getMST(self):
        ...
        # print(self.adjList, self.adjMat)
    ...
    def getTSPOptimal(self):
        ...
        # print('START RANK')
        # print(self.start.rank)
        numCities = len(self.adjList)
        paths = itertools.permutations(range(1, numCities))
        # print(paths)
        minDistance = math.inf
        for path in paths:
            distance = 0
            for j in range(len(path)-1):
                distance += self.adjMat[path[j]][path[j+1]]
            distance += self.adjMat[0][path[-1]]
