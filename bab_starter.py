...
class BBTreeNode():
    ...
    def bbsolve(self):
        ...
        root = self
        print(list(map(lambda x: (x.value), root.vars)))
        ...
        while (len(heap) > 0):
            ...
            solutionExists = True
            try:
                newRes = newNode.prob.solve(solver='cvxopt').value
            except:
                solutionExists = False
                pass

            if newNode.is_integral() and solutionExists:
                if newRes > bestres:
                    bestres = newRes
            ...
            if solutionExists:
                for var in newNode.vars[:-1]:
                    # print(var)
                    floorBranch = newNode.branch_floor(var)
                    # try:
                    #     floorRes = floorBranch.solve(solver='cvxopt')
                    #     print(floorRes.value)
                    heap.append((newRes, next(counter), floorBranch))
                    # except:
                    #     print('passed floor')
                    #     pass
                    ceilingBranch = newNode.branch_ceil(var)
                    # try:
                    #     ceilingRes = ceilingBranch.solve(solver='cvxopt').value
                    heap.append((newRes, next(counter), ceilingBranch))
                    # except:
                    #     print('passed ceil')
                    #     pass
        ...
        # print(res)
        # print(res.primals)
        # print(res.primals[self.vars[0]])
        # res.apply()
        # print(list(map(lambda x: (x.value), root.vars)))
        # print(root.is_integral())
        # # print(heap)
